...
- **Perl is a first-class Hybrid LSP language, not a fallback language.** `.pl` and `.pm` are indexed by extension (a `#!…perl` shebang also maps an extensionless file to Perl), and the Perl resolver handles packages, `@ISA` / `use parent` / `use base` inheritance with method-resolution-order dispatch, `SUPER::` calls, Exporter (`use Foo qw(...)`) import maps, `bless` / `ref($class)||$class` self-type inference, and qualified `Pkg::sub` static calls — with a zero-edge guarantee (an unresolved receiver emits no edge rather than a guessed one). Use `search_graph` / `trace_path` / `get_code_snippet` on Perl exactly as you would on Go or Python; do not reach for grep.
