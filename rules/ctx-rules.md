...
`PreToolUse`/`PostToolUse` hooks do not fire for tool calls made inside a subagent (Claude Code issue [#34692](https://github.com/anthropics/claude-code/issues/34692)), so the ctx anti-patterns above cannot be enforced on a lens agent's `Bash`/`Read` from the outside. A **Workflow-spawned worker** also bypasses `agent-cmm-gate.sh`. When authoring a Workflow or subagent, paste the block from `rules/cmm-agent-preamble.md` (installed to `.claude/rules/cmm-agent-preamble.md`) into every `agent()` prompt and `.claude/agents/*.md` definition — routing large diffs / test runs / multi-file scans through `ctx_execute` / `ctx_batch_execute` / `ctx_search` is what keeps a lens agent's output off *both* its own budget and the overall token budget. See [[cmm-rules]] "Subagents & Workflows".
