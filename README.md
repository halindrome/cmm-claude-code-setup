...
> On every `setup.sh --project` run, the merge finds each upstream entry by substring match on `hook claude-code <event>` and rewrites its `matcher` back to the upstream default if the two diverge. This keeps all installs on the same tool-coverage contract. **User customizations to the `matcher` field on these five entries are overwritten.** Flags appended to the npx launcher command (e.g. `--verbose`) are preserved. Legacy bare-form entries from early phase-51 installs (`context-mode hook claude-code <event>`, no `npx`) are rewritten in-place to the npx launcher form so the `--skip-mcp-check`-without-global-CLI scenario stops failing with exit 127. **User-pinned versions** like `npx -y context-mode@0.9.3 hook claude-code <event>` are preserved — the heal matches on the `context-mode@` prefix, not on `@latest` specifically, so your version pin survives re-runs. Matchers on your other (non-upstream) hook entries — including the CMM enforcement hooks and anything you added by hand — are never touched.

> **Per-call invocation cost.** Each hook fire invokes `npx -y context-mode@latest`. After the first invocation in a session, npm's on-disk cache serves subsequent calls with no network roundtrip, but `npx` process startup still adds tens-to-hundreds of milliseconds per tool call. If this overhead becomes an issue (e.g., very chatty sessions with many Bash/Read calls), install `context-mode` globally (`npm install -g context-mode`) — the hooks will still invoke `npx -y context-mode@latest`, but npx short-circuits to the global install when present. Global install is optional; the default npx-only path works out of the box.
