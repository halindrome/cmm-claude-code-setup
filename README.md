...
- **Agent deltas are gated.** The `agents/vbw-*.md` delta files are installed **only when VBW is available**. When VBW is not installed, setup injects nothing and moves any VBW artifacts from a prior install (`.claude/agents/vbw-*.md`, `.claude/agents-delta/vbw-*.md`) aside into `.claude/.cmm-setup/vbw-agents.bak/` (reversible). Install VBW and re-run `setup.sh` to enable the overrides.
