...
`PreToolUse` / `PostToolUse` hooks do **not** fire for tool calls made inside a subagent (Claude Code issue [#34692](https://github.com/anthropics/claude-code/issues/34692)), and a Workflow-spawned worker surfaces as a `Workflow` call rather than an `Agent` call, so it bypasses `agent-cmm-gate.sh` too. The subagent's **prompt** (and its `.claude/agents/*.md` definition) is therefore the only place CMM/ctx guidance provably lands — paste the block from `rules/cmm-agent-preamble.md` into every `agent()` prompt and agent definition. Do not rely on hooks to make lens agents use CMM.
